  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:8080/api/chat/getMessages/${username}/${sellerName}`);
      ...
      <div className="chatBoxTop">
        {messages?.map((msg, index) => (
          <div
            key={index}
            ref={scrollRef}
            className={isSender(msg.sender === userId ? user?.username : msg.sender) ? 'messageSender' : 'messageReceiver'}
          >
            <strong>{msg.sender === userId ? user?.username : msg.sender}: </strong>
            {msg.content}
          </div>
        ))}
      </div>
